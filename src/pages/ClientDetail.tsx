...
import { scoreClient } from '@/lib/crm';
...
import { ArrowLeft, FileText, StickyNote, Plus, Calendar, Trash2, Building2, Mail, Phone, MapPin, TrendingUp } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-6">
        {/* Scorecard */}
        {(() => {
          const s = scoreClient(clientDocs);
          const ratingStyle =
            s.rating === 'A' ? 'bg-success/20 text-success border-success/30'
            : s.rating === 'B' ? 'bg-accent/20 text-accent-foreground border-accent/30'
            : 'bg-muted text-muted-foreground';
          const stats = [
            { label: 'Lifetime revenue', value: formatCurrency(s.lifetimeRevenue) },
            { label: 'Jobs won', value: String(s.jobsWon) },
            { label: 'Win rate', value: `${s.winRate.toFixed(0)}%` },
            { label: 'Avg job size', value: formatCurrency(s.avgJobSize) },
            { label: 'Gross profit', value: formatCurrency(s.grossProfit) },
            { label: 'Margin', value: `${s.marginPct.toFixed(0)}%` },
            { label: 'Outstanding', value: formatCurrency(s.outstanding) },
            { label: 'Last activity', value: s.lastActivity ? format(new Date(s.lastActivity), 'dd MMM yyyy') : '—' },
          ];
          return (
            <Card className="p-4 sm:p-6 animate-fade-in">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-heading font-semibold text-base sm:text-lg flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 sm:h-5 sm:w-5" /> Client scorecard
                </h2>
                <Badge variant="outline" className={`${ratingStyle} font-heading font-bold`}>Grade {s.rating}</Badge>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {stats.map(stat => (
                  <div key={stat.label} className="rounded-lg border bg-muted/30 p-3">
                    ...
                    <p className="text-sm sm:text-base font-heading font-bold mt-1 truncate">{stat.value}</p>
                  </div>
                ))}
              </div>
            </Card>
