...
import { ArrowLeft, Save, Upload, X, Monitor, Moon, Sun, LogOut } from 'lucide-react';
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        <Tabs defaultValue="profile" className="space-y-6">
          ...
          <TabsContent value="appearance" className="space-y-6 mt-0">
            <Card className="p-6">
              <h2 className="font-heading font-semibold text-lg mb-1">Theme</h2>
              <p className="text-xs text-muted-foreground mb-4">
                Choose how HustleOS looks. Exported PDFs and JPEGs always stay light for printing.
              </p>
              {mounted && (
                <RadioGroup
                  value={theme ?? 'system'}
                ...
                >
                  {[
                    ...
                    <Label
                      key={value}
                      htmlFor={`theme-${value}`}
                      className="flex items-center gap-3 rounded-lg border p-4 cursor-pointer hover:bg-muted/50 has-[:checked]:border-accent has-[:checked]:bg-accent/10"
                    >
